...
enum DatabaseHandle {
    Sqlite(Connection),
    Network(DatabaseConnectionConfig),
}

struct RuntimeRecord {
    summary: DatabaseRuntimeSummary,
...
impl Default for DatabaseRuntimeManager {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DatabaseRuntimeManager {
    pub fn import_rows(
        ...
        if table.is_empty()
            ...
            || columns
                .iter()
                .any(|c| c.is_empty() || !c.chars().all(|x| x.is_ascii_alphanumeric() || x == '_'))
        {
            return Err("非法表名或字段名".into());
        }
        let mut map = self
            .runtimes
            .lock()
            .map_err(|_| "database runtime lock poisoned".to_string())?;
        let record = map
            .get_mut(id)
            .ok_or_else(|| "database runtime not found".to_string())?;
        let Some(DatabaseHandle::Sqlite(connection)) = record.handle.as_mut() else {
            return Err("database is disconnected".into());
        };
            ...
            let params = row
                ...
                .map(|v| match v {
                    ...
                    serde_json::Value::Number(n) => n
                        ...
                        .unwrap_or(rusqlite::types::Value::Text(n.to_string())),
    ...
    fn network_config(&self, id: &str) -> Result<Option<DatabaseConnectionConfig>, String> {
        ...
        let record = map
            .get(id)
            .ok_or_else(|| "database runtime not found".to_string())?;
        Ok(match record.handle.as_ref() {
            Some(DatabaseHandle::Network(config)) => Some(config.clone()),
            _ => None,
        })
    }
    pub fn import_sql(&self, id: &str, script: &str) -> Result<(), String> {
        if script.trim().is_empty() {
            return Err("SQL 脚本不能为空".into());
        }
        if let Some(config) = self.network_config(id)? {
            return tauri::async_runtime::block_on(import_network_sql(config, script));
        }
        let mut map = self
            .runtimes
            .lock()
        ...
        transaction.execute_batch(script).map_err(|e| {
            ...
            if message.contains("transaction within a transaction") {
                "脚本自带 BEGIN/COMMIT，无法整体执行：请移除脚本里的事务语句后重试".into()
            } else {
                message
            }
        })?;
    ...
    pub fn import_sql_file(&self, id: &str, path: &Path) -> Result<(), String> {
        let script =
            std::fs::read_to_string(path).map_err(|e| format!("无法读取 SQL 文件：{e}"))?;
    ...
    pub fn export_sql(
        ...
        path: &Path,
        table: Option<&str>,
    ) -> Result<DatabaseSqlExport, String> {
        self.export_sql_with_options(id, path, table, false)
    }
    #[allow(dead_code)]
    pub fn export_sql_with_options(
        &self,
        id: &str,
        path: &Path,
        table: Option<&str>,
    ...
    pub fn export_sql_with_progress(
        ...
        table: Option<&str>,
        schema_only: bool,
        progress: Option<DatabaseExportProgressSink>,
    ) -> Result<DatabaseSqlExport, String> {
        if let Some(config) = self.network_config(id)? {
            return tauri::async_runtime::block_on(export_network_sql(
                config,
                path,
                table,
                schema_only,
    ...
    pub fn export_csv(&self, id: &str, sql: &str) -> Result<String, String> {
        ...
        let mut out = result
            .columns
            .iter()
            .map(|c| quote(&serde_json::Value::String(c.clone())))
            .collect::<Vec<_>>()
            .join(",");
        out.push('\n');
    ...
    pub fn list_tables(&self, id: &str) -> Result<Vec<String>, String> {
        ...
        if let Some(DatabaseHandle::Network(config)) = record.handle.as_ref() {
            ...
            return tauri::async_runtime::block_on(execute_network(config, sql, 10000)).map(
                |result| {
                    result
                        .rows
                        .into_iter()
    ...
    pub fn connect(
    ...
    ) -> Result<DatabaseRuntimeSummary, String> {
        let id = Uuid::new_v4().to_string();
        let mut summary = DatabaseRuntimeSummary {
            id: id.clone(),
            driver: config.driver.clone(),
            status: DatabaseRuntimeStatus::Connecting,
            error: None,
        };
        let handle = match config.driver {
            DatabaseDriver::Sqlite => {
                let path = config
                    .sqlite_path
                    .ok_or_else(|| "SQLite connection requires sqlitePath".to_string())?;
                ...
                Connection::open_with_flags(path, flags)
                    .map(DatabaseHandle::Sqlite)
                    .map_err(|e| e.to_string())
            }
            DatabaseDriver::Mysql | DatabaseDriver::Mariadb | DatabaseDriver::Postgres => {
                tauri::async_runtime::block_on(validate_network_config(config.clone()))
                    .map(|_| DatabaseHandle::Network(config.clone()))
            }
        };
        ...
        self.runtimes
            .lock()
            .map_err(|_| "database runtime lock poisoned".to_string())?
            .insert(id, RuntimeRecord { summary, handle });
        Ok(result)
    }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DatabaseSqlExportProgress {
...
pub fn write_query_sql_file_with_progress(
    path: &Path,
    sql: &str,
    rows: u64,
    progress: Option<DatabaseExportProgressSink>,
    ...
    if sql.trim().is_empty() {
        return Err("查询结果为空，无法导出 SQL 文件".into());
    }
...
pub struct DatabaseColumnInfo {
    ...
    /// Declared size of the type as its engine reports it: the character length
    /// (`255`), or `precision,scale` for the decimal family (`10,2`). `None` for types
    /// that take no size, and rendered as an em dash by the structure view.
    #[serde(default)]
...
/// Reads an integer-valued metadata cell.
///
/// Both drivers reach this query over a text protocol, so every value arrives as a JSON
/// string today: `read_postgres_result` stringifies each one, and `mysql_value` maps the
/// text protocol's bytes to a string. The number arm is defensive only — `mysql_value`
/// does turn `Value::Int`/`Value::UInt` into JSON numbers, so the shape is a property of
/// the driver rather than of this query, and reading both keeps a future switch to the
/// binary protocol from silently emptying the length column.
fn row_i64(value: Option<&serde_json::Value>) -> Option<i64> {
    match value? {
        // Metadata that cannot be a size is not a size, so an out-of-range integer is
...
/// Declared size of a column: its character maximum length when the type takes one,
/// otherwise `precision,scale` for the decimal family.
///
/// The type-name gate is load-bearing rather than defensive: PostgreSQL reports
/// `numeric_precision = 32, numeric_scale = 0` for a plain `integer`, so reading width
/// from precision alone would render every integer column as `32,0`.
fn column_size(
    data_type: &str,
    ...
    // MySQL reports the longest member for these, which is not a declared length.
    ...
    if matches!(kind.as_str(), "decimal" | "numeric" | "dec" | "fixed") {
        if let (Some(precision), Some(scale)) = (numeric_precision, numeric_scale) {
            return Some(format!("{precision},{scale}"));
        }
    }
    None
}

/// SQLite keeps no separate size column: the declared type text carries it, as in
/// `VARCHAR(255)` or `DECIMAL(10,2)`. Only a purely numeric parenthetical counts as a
/// size, so `UNSIGNED BIG INT` and friends report none.
fn sqlite_declared_size(declared_type: &str) -> Option<String> {
    let inner = declared_type.split_once('(')?.1.split_once(')')?.0.trim();
    if !inner
        .chars()
    ...
    let size: String = inner.chars().filter(|c| !c.is_whitespace()).collect();
    size.chars().any(|c| c.is_ascii_digit()).then_some(size)
}
...
impl DatabaseRuntimeManager {
    pub fn list_foreign_keys(
        ...
        id: &str,
        table: &str,
    ) -> Result<Vec<DatabaseForeignKeyInfo>, String> {
        if !table.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            return Err("非法表名".into());
        }
        let mut map = self
            .runtimes
            .lock()
        ...
        let mut s = connection
            ...
            .map_err(|e| e.to_string())?;
        s.query_map([], |r| {
            Ok(DatabaseForeignKeyInfo {
                references_table: r.get(2)?,
                column: r.get(3)?,
                references_column: r.get(4)?,
                on_update: r.get(5)?,
                on_delete: r.get(6)?,
            })
        })
    ...
    pub fn list_indexes(&self, id: &str, table: &str) -> Result<Vec<DatabaseIndexInfo>, String> {
        ...
        if let Some(DatabaseHandle::Network(config)) = record.handle.as_ref() {
            ...
            drop(map);
            let sql = format!(
                "SELECT index_name, CASE WHEN non_unique=0 THEN 1 ELSE 0 END FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='{}' GROUP BY index_name,non_unique ORDER BY index_name",
                table
            );
            return tauri::async_runtime::block_on(execute_network(config, &sql, 10000)).map(|r| {
                r.rows
                    .into_iter()
                    .filter_map(|row| {
                        Some(DatabaseIndexInfo {
                            name: row.first()?.as_str()?.into(),
                            unique: row.get(1).and_then(|v| v.as_str()) == Some("1"),
                        })
                    })
                    .collect()
            });
        ...
        let mut s = connection
            ...
            .map_err(|e| e.to_string())?;
        s.query_map([], |r| {
            Ok(DatabaseIndexInfo {
                name: r.get(1)?,
                unique: r.get::<_, i64>(2)? != 0,
            })
        })
        .map_err(|e| e.to_string())?
        .collect::<rusqlite::Result<Vec<_>>>()
    ...
    pub fn create_index(
        ...
        for value in [table, index, column] {
            if value.is_empty() || !value.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
                return Err("非法索引名称".into());
            }
        }
        let mut map = self
            .runtimes
    ...
    pub fn drop_index(&self, id: &str, index: &str) -> Result<u64, String> {
        if index.is_empty() || !index.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            return Err("非法索引名称".into());
        }
        let mut map = self
            .runtimes
    ...
    pub fn describe_table(&self, id: &str, table: &str) -> Result<Vec<DatabaseColumnInfo>, String> {
        ...
        if let Some(DatabaseHandle::Network(config)) = record.handle.as_ref() {
            ...
            drop(map);
            let sql = match config.driver {
                // The trailing size and primary-key columns are read positionally by the
                // shared row closure below, so both branches have to select them in the
                // same order as the character columns.
                //
                // Both engines report the primary key the same way here: MySQL has a
                // dedicated `column_key` column, and the left join below projects
                // PostgreSQL's key column usage into the same `PRI` marker, so one
                // comparison serves both.
                DatabaseDriver::Postgres => format!(
                    "SELECT c.column_name,c.data_type,c.is_nullable,c.column_default,pg_catalog.col_description(cl.oid,attr.attnum),c.character_maximum_length,c.numeric_precision,c.numeric_scale,CASE WHEN kcu.column_name IS NULL THEN '' ELSE 'PRI' END FROM information_schema.columns c JOIN pg_catalog.pg_namespace ns ON ns.nspname=c.table_schema JOIN pg_catalog.pg_class cl ON cl.relnamespace=ns.oid AND cl.relname=c.table_name JOIN pg_catalog.pg_attribute attr ON attr.attrelid=cl.oid AND attr.attname=c.column_name LEFT JOIN information_schema.table_constraints tc ON tc.table_schema=c.table_schema AND tc.table_name=c.table_name AND tc.constraint_type='PRIMARY KEY' LEFT JOIN information_schema.key_column_usage kcu ON kcu.constraint_schema=tc.constraint_schema AND kcu.constraint_name=tc.constraint_name AND kcu.table_schema=c.table_schema AND kcu.table_name=c.table_name AND kcu.column_name=c.column_name WHERE c.table_schema='public' AND c.table_name='{}' ORDER BY c.ordinal_position",
                    table
                ),
                _ => format!(
                    "SELECT column_name,data_type,is_nullable,column_default,column_comment,character_maximum_length,numeric_precision,numeric_scale,column_key FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='{}' ORDER BY ordinal_position",
                    table
                ),
            };
            return tauri::async_runtime::block_on(execute_network(config, &sql, 10000)).map(
                    ...
                    result
                        ...
                        .map(|row| {
                            ...
                            DatabaseColumnInfo {
                                ...
                                data_type,
                                not_null: row.get(2).and_then(|v| v.as_str()) == Some("NO"),
                                primary_key: row.get(8).and_then(|v| v.as_str()) == Some("PRI"),
                                default_value: row
                                    .get(3)
                                    .and_then(|v| v.as_str())
                                    .map(str::to_string),
        ...
        s.query_map([], |r| {
            ...
            Ok(DatabaseColumnInfo {
                name: r.get(1)?,
                length: sqlite_declared_size(&data_type),
                data_type,
    ...
    pub fn execute_page(
        ...
        if page_offset > 0 && paged_sql == sql && !pageable {
            return Err("此 SQL 不支持自动翻页，请修改 SQL 后执行".into());
        }
    ...
    pub fn execute(
        ...
        let DatabaseHandle::Sqlite(connection) = handle else {
            return Err("database is disconnected".into());
        };
        ...
        while let Some(row) = cursor.next().map_err(|e| e.to_string())? {
            ...
            for index in 0..count {
                ...
                let value = match row.get_ref(index).map_err(|e| e.to_string())? {
                    ...
                    ValueRef::Text(v) => {
                        serde_json::json!(String::from_utf8_lossy(&v[..v.len().min(65536)]))
                    }
        ...
        Ok(DatabaseQueryResult {
            columns,
            truncated: truncated || rows.len() == limit,
            rows,
            affected_rows: 0,
            has_more: None,
        })
...
async fn execute_network(
    ...
    tokio::time::timeout(QUERY_TIMEOUT, execute_network_inner(config, sql, max_rows))
        .await
        .map_err(|_| {
            "数据库查询超时（30 秒）；执行结果可能未知，请刷新数据后再决定是否重试".to_string()
        })?
}
...
async fn execute_network_inner(
    ...
    let host = config
        .host
        .clone()
        .ok_or_else(|| "数据库主机不能为空".to_string())?;
    let port = config
        .port
        .unwrap_or(if matches!(config.driver, DatabaseDriver::Postgres) {
            5432
        } else {
            3306
        });
    ...
    match config.driver {
        DatabaseDriver::Mysql | DatabaseDriver::Mariadb => {
            ...
            let conn = tokio::time::timeout(CONNECTION_TIMEOUT, mysql_async::Conn::new(builder))
                .await
                .map_err(|_| "数据库连接超时（8 秒）".to_string())?
                .map_err(|e| e.to_string())?;
            let mut connection = MysqlConnection(Some(conn));
            let conn = connection
                .0
                .as_mut()
                .expect("connection is owned until cleanup");
...
fn prepare_query(
    ...
    use sqlparser::{
        ast::{Expr, LimitClause, Query, SetExpr, Statement, Value},
        dialect::{Dialect, MySqlDialect, PostgreSqlDialect, SQLiteDialect},
        parser::Parser,
    };
    fn read_query(query: &Query) -> bool {
        query.locks.is_empty()
            && read_body(&query.body)
            && query
                .with
                .as_ref()
                .is_none_or(|w| w.cte_tables.iter().all(|c| read_query(&c.query)))
    }
        ...
        match expr {
            Expr::Value(value) => match &value.value {
                Value::Number(number, _) => match number.parse::<u64>() {
                    ...
                    Ok(_) => {
                        *expr = Expr::Value(Value::Number(cap.to_string(), false).into());
                        true
                    }
                    Err(_) => false,
                },
            ...
            Expr::UnaryOp {
                ...
                expr: inner,
            } if matches!(inner.as_ref(), Expr::Value(v) if matches!(&v.value, Value::Number(n, _) if n == "1")) =>
            {
                *expr = Expr::Value(Value::Number(cap.to_string(), false).into());
                true
            }
    ...
    let dialect: Box<dyn Dialect> = match driver {
        DatabaseDriver::Sqlite => Box::new(SQLiteDialect {}),
        DatabaseDriver::Postgres => Box::new(PostgreSqlDialect {}),
        _ => Box::new(MySqlDialect {}),
    };
    let Ok(mut statements) = Parser::parse_sql(dialect.as_ref(), sql) else {
        return (sql.into(), false);
    };
    if statements.len() != 1 {
        return (sql.into(), false);
    }
    ...
    fn sqlite_accepts_limit(body: &SetExpr) -> bool {
        match body {
            ...
            SetExpr::SetOperation { right, .. } => sqlite_accepts_limit(right),
            _ => false,
        }
    }
    if matches!(driver, DatabaseDriver::Sqlite) && !sqlite_accepts_limit(&query.body) {
        return (sql.into(), false);
    }
    if let Some(page_offset) = page_offset {
        ...
        let (total, base_offset) = match &query.limit_clause {
            ...
            Some(LimitClause::LimitOffset { limit, offset, .. }) => {
                let total = match limit {
                    None => None,
                    Some(expr) => match number(expr) {
                        Some(n) => Some(n),
                        None => return (sql.into(), false),
                    },
                };
                let offset = match offset {
                    None => 0,
                    Some(offset) => match number(&offset.value) {
                        Some(n) => n,
                        None => return (sql.into(), false),
                    },
                };
            ...
            Some(LimitClause::OffsetCommaLimit { offset, limit }) => {
                match (number(limit), number(offset)) {
                    (Some(limit), Some(offset)) => (Some(limit), offset),
                    _ => return (sql.into(), false),
                }
            }
        };
        let count = total
            .map(|total| total.saturating_sub(page_offset).min(cap))
            .unwrap_or(cap);
        let Some(offset) = base_offset.checked_add(page_offset) else {
            return (sql.into(), false);
        };
        let literal = |n: usize| Expr::Value(Value::Number(n.to_string(), false).into());
        query.limit_clause = Some(match driver {
            DatabaseDriver::Mysql | DatabaseDriver::Mariadb => LimitClause::OffsetCommaLimit {
                offset: literal(offset),
                limit: literal(count),
            },
            _ => LimitClause::LimitOffset {
                limit: Some(literal(count)),
        ...
        return (
            ...
            total.is_none_or(|n| n.saturating_sub(page_offset) > cap),
        );
    }
    let changed = if let Some(fetch) = &mut query.fetch {
        if fetch.with_ties || fetch.percent {
            ...
            fetch
                .quantity
                .as_mut()
                .is_some_and(|expr| clamp_literal(expr, cap))
        }
    } else {
        match &mut query.limit_clause {
            Some(LimitClause::LimitOffset { limit, .. }) => {
                if let Some(expr) = limit {
                    clamp_literal(expr, cap)
                } else {
                    *limit = Some(Expr::Value(Value::Number(cap.to_string(), false).into()));
                    true
                }
            }
...
async fn read_postgres_result(
    client: &tokio_postgres::Client,
    sql: &str,
    limit: usize,
) -> Result<DatabaseQueryResult, String> {
    use futures::TryStreamExt;
    let messages = client.simple_query_raw(sql).await.map_err(|e| {
        e.as_db_error()
...
async fn validate_network_config(config: DatabaseConnectionConfig) -> Result<(), String> {
    tokio::time::timeout(
        CONNECTION_TIMEOUT,
        execute_network_inner(config, "SELECT 1", 1),
    )
    .await
    .map_err(|_| "数据库连接测试超时（8 秒）".to_string())?
    .map(|_| ())
}
...
fn mysql_value(value: mysql_async::Value) -> serde_json::Value {
    match value {
        ...
        mysql_async::Value::Int(v) => serde_json::json!(v),
        mysql_async::Value::UInt(v) => serde_json::json!(v),
        mysql_async::Value::Float(v) => serde_json::json!(v),
        mysql_async::Value::Double(v) => serde_json::json!(v),
        mysql_async::Value::Date(y, m, d, h, mi, s, ms) => serde_json::json!(format!(
            "{y:04}-{m:02}-{d:02} {h:02}:{mi:02}:{s:02}.{ms:03}"
        )),
        mysql_async::Value::Time(..) => serde_json::json!(format!("{value:?}")),
    }
}
...
fn sqlite_dump_objects(
    ...
    let filter =
        " AND (name = ?1 OR (tbl_name = ?1 AND type IN ('index', 'trigger'))) ORDER BY rowid";
    let mut statement = connection
        .prepare(&match table {
            Some(_) => format!("{BASE}{filter}"),
            None => format!("{BASE} ORDER BY rowid"),
        })
        .map_err(|e| e.to_string())?;
    let project = |row: &rusqlite::Row<'_>| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, String>(1)?,
            row.get::<_, String>(2)?,
    ...
    let rows = match table {
        Some(name) => statement.query_map([name], project),
        None => statement.query_map([], project),
    }
    .map_err(|e| e.to_string())?;
...
fn write_sql_dump(
    ...
    if objects.is_empty() && table.is_some() {
        return Err("数据库中找不到该表或视图".into());
    }
    ...
    for (index, (_, name, sql)) in tables.iter().enumerate() {
        ...
        if schema_only {
            if let Some(report) = progress {
                report(index + 1, total_tables, Some(name));
            }
        }
    }
    if !tables.is_empty() {
...
fn write_sql_table_data(
    connection: &Connection,
    out: &mut impl Write,
    table: &str,
) -> Result<u64, String> {
    let mut statement = connection
        .prepare(&format!("SELECT * FROM {}", sql_dump_identifier(table)))
        .map_err(|e| e.to_string())?;
    let columns = statement
        .column_names()
        .iter()
        .map(|name| sql_dump_identifier(name))
        .collect::<Vec<_>>();
    let mut cursor = statement.query([]).map_err(|e| e.to_string())?;
    ...
    while let Some(row) = cursor.next().map_err(|e| e.to_string())? {
        ...
        batch += 1;
        written += 1;
        if batch == SQL_DUMP_ROWS_PER_STATEMENT {
            out.write_all(b";\n").map_err(|e| e.to_string())?;
            batch = 0;
        }
    }
...
fn sql_dump_identifier(name: &str) -> String {
    format!("\"{}\"", name.replace('"', "\"\""))
}
...
fn write_sql_value(out: &mut impl Write, value: ValueRef<'_>) -> Result<(), String> {
    match value {
        ...
        ValueRef::Real(number) => {
            if number.is_finite() {
                write!(out, "{number:?}").map_err(|e| e.to_string())
            } else {
...
fn finish_dump(mut out: BufWriter<File>) -> Result<u64, String> {
    ...
    let file = out
        .into_inner()
        .map_err(|e| format!("无法写入导出文件：{}", e.into_error()))?;
    file.metadata()
        .map(|meta| meta.len())
        .map_err(|e| e.to_string())
}

fn sql_identifier(quote: char, name: &str) -> String {
    format!(
        "{quote}{}{quote}",
...
async fn export_network_sql(
    ...
    match config.driver {
        DatabaseDriver::Sqlite => Err("SQLite 使用本地连接".into()),
        DatabaseDriver::Postgres => {
            postgres_export_sql_with_progress(config, path, table, schema_only, progress).await
        }
        DatabaseDriver::Mysql | DatabaseDriver::Mariadb => {
            mysql_export_sql_with_progress(config, path, table, schema_only, progress).await
        }
    }
}
...
async fn import_network_sql(config: DatabaseConnectionConfig, script: &str) -> Result<(), String> {
    match config.driver {
        ...
        DatabaseDriver::Postgres => {
            let (client, _connection_task) = postgres_client(&config).await?;
            client
                .batch_execute(&format!("SET client_encoding = 'UTF8';\n{script}"))
                .await
...
async fn mysql_client(config: &DatabaseConnectionConfig) -> Result<mysql_async::Conn, String> {
    ...
    tokio::time::timeout(CONNECTION_TIMEOUT, mysql_async::Conn::new(builder))
        .await
        .map_err(|_| "数据库连接超时（8 秒）".to_string())?
        .map_err(|e| e.to_string())
}
...
fn mysql_value_kind(column: &mysql_async::Column) -> MysqlValueKind {
    ...
    match column.column_type() {
        ...
        | MYSQL_TYPE_TIME
        | MYSQL_TYPE_TIME2 => MysqlValueKind::Text,
...
struct MysqlDumpObject {
    name: String,
    is_view: bool,
}

async fn mysql_dump_objects(
    connection: &mut mysql_async::Conn,
    table: Option<&str>,
) -> Result<Vec<MysqlDumpObject>, String> {
    ...
    let rows = connection
        .query::<(String, String), _>(sql)
        .await
        .map_err(|e| e.to_string())?;
    Ok(rows
        .into_iter()
        .map(|(name, kind)| MysqlDumpObject {
            name,
...
async fn mysql_object_ddl(
    connection: &mut mysql_async::Conn,
    object: &MysqlDumpObject,
) -> Result<String, String> {
    let statement = if object.is_view {
        "SHOW CREATE VIEW"
    } else {
        "SHOW CREATE TABLE"
    };
...
async fn mysql_write_table_data(
    connection: &mut mysql_async::Conn,
    out: &mut impl Write,
    table: &str,
) -> Result<u64, String> {
    let mut result = connection
        .query_iter(format!("SELECT * FROM {}", sql_identifier('`', table)))
        .await
        .map_err(|e| e.to_string())?;
    let columns = result.columns_ref().to_vec();
    let names = columns
        .iter()
        .map(|column| sql_identifier('`', &column.name_str()))
        .collect::<Vec<_>>()
        .join(", ");
    let mut written = 0u64;
    let mut batch = 0usize;
    while let Some(row) = result.next().await.map_err(|e| e.to_string())? {
        ...
        for (index, value) in values.iter().enumerate() {
            ...
            let kind = columns
                .get(index)
                .map(mysql_value_kind)
                .unwrap_or(MysqlValueKind::Text);
...
fn mysql_dump_value(
    ...
    match value {
        ...
        Value::Date(year, month, day, hour, minute, second, micros) => mysql_dump_string(
            out,
            mysql_datetime_text(*year, *month, *day, *hour, *minute, *second, *micros).as_bytes(),
        ),
        Value::Time(negative, days, hours, minutes, seconds, micros) => mysql_dump_string(
            out,
            mysql_time_text(*negative, *days, *hours, *minutes, *seconds, *micros).as_bytes(),
        ),
    }
}
...
async fn mysql_export_sql_with_progress(
    config: DatabaseConnectionConfig,
    path: &Path,
    table: Option<&str>,
    schema_only: bool,
...
async fn postgres_client(
    config: &DatabaseConnectionConfig,
) -> Result<(tokio_postgres::Client, ConnectionTask), String> {
    let host = config
        .host
        .clone()
        .ok_or_else(|| "数据库主机不能为空".to_string())?;
...
fn postgres_quote_ident(name: &str) -> String {
    sql_identifier('"', name)
}

/// `regclass` parses its input like an identifier, so the quoted name keeps the exact case.
fn postgres_relclass(name: &str) -> String {
    format!(
        "{}::regclass",
        sql_string_literal(&postgres_quote_ident(name))
    )
}

fn postgres_text_rows(
...
fn text_at(row: &[Option<String>], index: usize) -> Option<String> {
...
async fn postgres_query_rows(
    ...
    client
        .simple_query(sql)
        .await
        .map(postgres_text_rows)
        .map_err(postgres_error)
}

struct PostgresDumpObject {
    name: String,
    is_view: bool,
}
...
impl PostgresColumn {
    ...
    fn definition(&self) -> String {
        let mut definition = format!(
            "  {} {}",
            postgres_quote_ident(&self.name),
            self.serial.unwrap_or(&self.type_sql)
        );
        if self.identity {
        ...
        if self.serial.is_none() && !self.identity {
    ...
    fn needs_sequence_reset(&self) -> bool {
        self.identity || self.serial.is_some()
    }
}

async fn postgres_dump_objects(
    client: &tokio_postgres::Client,
    table: Option<&str>,
) -> Result<Vec<PostgresDumpObject>, String> {
    let mut sql = String::from(
    ...
    sql.push_str(" ORDER BY table_type, table_name");
    Ok(postgres_query_rows(client, &sql)
        .await?
        .into_iter()
        .map(|row| PostgresDumpObject {
...
async fn postgres_table_columns(
    client: &tokio_postgres::Client,
    table: &str,
) -> Result<Vec<PostgresColumn>, String> {
    let sql = format!(
        "SELECT a.attname, format_type(a.atttypid, a.atttypmod), a.attnotnull, a.attidentity, pg_get_expr(d.adbin, d.adrelid) \
        FROM pg_attribute a LEFT JOIN pg_attrdef d ON d.adrelid = a.attrelid AND d.adnum = a.attnum \
        WHERE a.attrelid = {} AND a.attnum > 0 AND NOT a.attisdropped ORDER BY a.attnum",
        postgres_relclass(table)
    );
    Ok(postgres_query_rows(client, &sql)
        .await?
        .into_iter()
...
fn postgres_serial_type(type_sql: &str, default: Option<&str>) -> Option<&'static str> {
    if !default.is_some_and(|default| default.starts_with("nextval(")) {
        return None;
    }
    match type_sql {
...
async fn postgres_table_indexes(
    client: &tokio_postgres::Client,
    table: &str,
) -> Result<Vec<String>, String> {
    // Constraint-backed indexes are recreated with their constraint, so they are skipped here.
    let sql = format!(
        "SELECT pg_get_indexdef(i.indexrelid) FROM pg_index i WHERE i.indrelid = {} AND NOT i.indisprimary \
        AND NOT EXISTS (SELECT 1 FROM pg_constraint c WHERE c.conindid = i.indexrelid) ORDER BY i.indexrelid",
        postgres_relclass(table)
    );
    Ok(postgres_query_rows(client, &sql)
        .await?
...
async fn postgres_table_constraints(
    client: &tokio_postgres::Client,
    table: &str,
) -> Result<Vec<(String, String, String)>, String> {
    let sql = format!(
        "SELECT conname, contype::text, pg_get_constraintdef(oid) FROM pg_constraint WHERE conrelid = {} \
        AND contype IN ('p','u','c','f') ORDER BY contype, conname",
        postgres_relclass(table)
    );
    Ok(postgres_query_rows(client, &sql)
        .await?
        .into_iter()
        .filter_map(
            |row| match (text_at(&row, 0), text_at(&row, 1), text_at(&row, 2)) {
...
async fn postgres_write_table_data(
    client: &tokio_postgres::Client,
    out: &mut impl Write,
    table: &str,
    ...
    use futures::TryStreamExt;
    let messages = client
        .simple_query_raw(&format!("SELECT * FROM {}", postgres_quote_ident(table)))
        .await
        .map_err(postgres_error)?;
    futures::pin_mut!(messages);
    ...
    while let Some(message) = messages.try_next().await.map_err(postgres_error)? {
        ...
        let names = names
            .as_ref()
            .ok_or_else(|| "查询结果缺少列信息".to_string())?;
        if batch == 0 {
            write!(
                out,
                "INSERT INTO {} ({}) VALUES\n  ",
                postgres_quote_ident(table),
                names
                    .iter()
                    .map(|name| postgres_quote_ident(name))
                    .collect::<Vec<_>>()
                    .join(", ")
            )
            .map_err(|e| e.to_string())?;
        ...
        for (index, name) in names.iter().enumerate() {
            ...
            let bytea = columns
                ...
                .is_some_and(PostgresColumn::is_bytea);
...
fn postgres_dump_value(
    out: &mut impl Write,
    value: Option<&str>,
    bytea: bool,
) -> Result<(), String> {
    let Some(text) = value else {
        return out.write_all(b"NULL").map_err(|e| e.to_string());
    };
    let escaped = bytea || text.contains('\\');
    out.write_all(if escaped { b"E'" } else { b"'" })
        .map_err(|e| e.to_string())?;
    let mut start = 0;
...
fn postgres_sequence_reset(table: &str, column: &PostgresColumn) -> String {
    ...
    format!(
        "SELECT setval(pg_get_serial_sequence({}, {}), COALESCE(MAX({name}), 1), COUNT({name}) > 0) FROM {};",
        sql_string_literal(&postgres_quote_ident(table)),
...
async fn postgres_export_sql_with_progress(
    config: DatabaseConnectionConfig,
    path: &Path,
    table: Option<&str>,
    schema_only: bool,
    ...
    for (index, object) in tables.iter().enumerate() {
        ...
        writeln!(
            ...
            postgres_quote_ident(&object.name)
        )
        .map_err(|e| e.to_string())?;
        writeln!(out, "CREATE TABLE {} (", postgres_quote_ident(&object.name))
            .map_err(|e| e.to_string())?;
        writeln!(
            out,
            "{}",
        ...
        if schema_only {
            if let Some(report) = progress.as_ref() {
                report(index + 1, total_tables, Some(&object.name));
            }
        }
    ...
    for (name, columns) in &dumped {
        for index in postgres_table_indexes(&client, name).await? {
            writeln!(out, "{index};").map_err(|e| e.to_string())?;
        }
        for (constraint, _, definition) in postgres_table_constraints(&client, name)
            .await?
            .iter()
            .filter(|(_, kind, _)| kind != "f")
        {
            writeln!(
                ...
                postgres_quote_ident(name),
                postgres_quote_ident(constraint)
            )
            .map_err(|e| e.to_string())?;
        }
        if !schema_only {
            for column in columns
                .iter()
                .filter(|column| column.needs_sequence_reset())
            {
                writeln!(out, "{}", postgres_sequence_reset(name, column))
                    .map_err(|e| e.to_string())?;
            }
        }
    }
    ...
    for object in &views {
        writeln!(
            ...
            postgres_quote_ident(&object.name),
            postgres_quote_ident(&object.name),
            postgres_view_definition(&client, &object.name)
                .await?
                .trim_end_matches(';')
        )
        .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    async fn mysql_rows_server(
        ...
        greeting.extend_from_slice(b"8.0.36-fixture\0");
        greeting.extend_from_slice(&1u32.to_le_bytes());
        greeting.extend_from_slice(b"12345678\0");
        greeting.extend_from_slice(&(caps as u16).to_le_bytes());
        greeting.push(45);
        greeting.extend_from_slice(&2u16.to_le_bytes());
        greeting.extend_from_slice(&((caps >> 16) as u16).to_le_bytes());
        greeting.push(21);
        ...
        while let Ok(data) = read_packet(&mut stream).await {
            ...
            let values = if sql.contains("@@max_allowed_packet") {
                vec![Some("67108864"), Some("28800")]
            } else if sql.contains("column_comment") {
                assert!(sql.contains("character_maximum_length"));
                assert!(sql.contains("numeric_precision"));
                assert!(sql.contains("numeric_scale"));
                assert!(sql.contains("column_key"));
                // A declared varchar primary key, whose length comes from the character
                // maximum.
                vec![
                    Some("name"),
                    Some("varchar"),
                ...
                if stall_query {
                    ...
                    return read_packet(&mut stream)
                        .await
                        .map(|data| data.first() == Some(&1))
                        .unwrap_or(true);
                }
                vec![Some("1")]
            };
            ...
            for _ in &values {
                ...
                column.extend_from_slice(&20u32.to_le_bytes());
                column.extend_from_slice(&[3, 0, 0, 0, 0, 0]);
                packet(&mut stream, sequence, &column).await;
                sequence += 1;
            }
            ...
            let mut row = Vec::new();
            for value in values {
                // 0xfb is the length-encoded NULL of the text protocol.
                match value {
                    Some(value) => {
            ...
            for _ in 0..if sql.starts_with("SELECT 1") {
                capped_rows.unwrap_or(1)
            } else {
                1
            } {
                packet(&mut stream, sequence, &row).await;
                sequence = sequence.wrapping_add(1);
            }
            if sql.starts_with("SELECT 1") && late_error {
                let mut error = vec![255, 85, 5];
                error.extend_from_slice(b"#22012late database error");
            ...
            if sql.starts_with("SELECT 1") && capped_rows.is_some() {
    ...
    async fn mysql_row_budget_is_applied_on_server_and_completed() {
        for driver in [DatabaseDriver::Mysql, DatabaseDriver::Mariadb] {
            ...
            let result = tokio::time::timeout(
                ...
                execute_network(config, "SELECT 1", 500),
            )
            .await
            .expect("bounded SELECT must finish")
            .unwrap();
    ...
    async fn mysql_column_comments_survive_native_metadata_path() {
        for driver in [DatabaseDriver::Mysql, DatabaseDriver::Mariadb] {
            ...
            manager.runtimes.lock().unwrap().insert(
                "fixture".into(),
                RuntimeRecord {
                    summary: DatabaseRuntimeSummary {
                        id: "fixture".into(),
                        driver,
                        status: DatabaseRuntimeStatus::Ready,
                        error: None,
                    },
            ...
            let columns =
                tokio::task::spawn_blocking(move || manager.describe_table("fixture", "users"))
                    .await
                    .unwrap()
                    .unwrap();
            assert_eq!(columns[0].comment.as_deref(), Some("用户名"));
            assert_eq!(columns[0].length.as_deref(), Some("255"));
    ...
    fn query_budget_preserves_write_batches_and_existing_limits() {
        for driver in [
            ...
            assert!(
                bounded_query_sql("SELECT * FROM users; -- trailing", &driver, 500)
                    .ends_with("LIMIT 500")
            );
            assert!(
                bounded_query_sql(
            ...
            assert!(
                bounded_query_sql("SELECT * FROM users LIMIT 900 OFFSET 12", &driver, 500)
                    .ends_with("LIMIT 500 OFFSET 12")
            );
            for sql in [
                ...
                assert_eq!(bounded_query_sql(sql, &driver, 500), sql);
            }
        }
        for sql in [
            "SELECT * INTO copied FROM users",
            "WITH d AS (DELETE FROM users RETURNING *) SELECT * FROM d",
            "INSERT INTO users SELECT * FROM archive RETURNING *",
            "SELECT * FROM users LIMIT $1",
        ] {
            assert_eq!(bounded_query_sql(sql, &DatabaseDriver::Postgres, 500), sql);
        }
        assert!(
            bounded_query_sql(
                "SELECT * FROM users LIMIT 20, 900",
                &DatabaseDriver::Mysql,
                500
            )
            .ends_with("LIMIT 20, 500")
        );
        assert!(
            bounded_query_sql("SELECT * FROM users LIMIT -1", &DatabaseDriver::Sqlite, 500)
                .ends_with("LIMIT 500")
        );
        assert!(
            bounded_query_sql(
                "SELECT * FROM users FETCH FIRST 900 ROWS ONLY",
                &DatabaseDriver::Postgres,
                500
            )
            .ends_with("FETCH FIRST 500 ROWS ONLY")
        );
    }

    #[test]
    fn column_size_reads_only_declared_sizes() {
        // Character types carry their length directly.
        assert_eq!(
            column_size("varchar", Some(255), None, None).as_deref(),
            Some("255")
        );
        assert_eq!(
            column_size("character", Some(10), None, None).as_deref(),
            Some("10")
        );
        // The decimal family reports precision and scale instead.
        assert_eq!(
            column_size("numeric", None, Some(10), Some(2)).as_deref(),
            Some("10,2")
        );
        assert_eq!(
            column_size("DECIMAL", None, Some(18), Some(0)).as_deref(),
            Some("18,0")
        );
        // PostgreSQL describes integer, bigint and the float types with real
        // precision/scale numbers that are not declared sizes.
        assert_eq!(column_size("integer", None, Some(32), Some(0)), None);
        assert_eq!(column_size("bigint", None, Some(64), Some(0)), None);
        assert_eq!(column_size("smallint", None, Some(16), Some(0)), None);
        assert_eq!(column_size("double precision", None, Some(53), None), None);
        assert_eq!(column_size("real", None, Some(24), None), None);
        assert_eq!(column_size("text", None, None, None), None);
        assert_eq!(column_size("timestamptz", None, None, None), None);
        // A decimal without both halves is not a size.
        assert_eq!(column_size("numeric", None, Some(10), None), None);
        // MySQL reports the longest member for these, which is not a declared length.
        assert_eq!(column_size("enum", Some(4), None, None), None);
        assert_eq!(column_size("set", Some(7), None, None), None);
    }

    #[test]
    fn sqlite_declared_size_reads_the_declared_parenthetical() {
        assert_eq!(sqlite_declared_size("VARCHAR(255)").as_deref(), Some("255"));
        assert_eq!(
            sqlite_declared_size("DECIMAL(10,5)").as_deref(),
            Some("10,5")
        );
        assert_eq!(
            sqlite_declared_size("varchar ( 20 )").as_deref(),
            Some("20")
        );
        assert_eq!(sqlite_declared_size("TEXT"), None);
        assert_eq!(sqlite_declared_size("INTEGER"), None);
        assert_eq!(sqlite_declared_size("UNSIGNED BIG INT"), None);
        assert_eq!(sqlite_declared_size(""), None);
        // Only a purely numeric parenthetical is a size.
        assert_eq!(sqlite_declared_size("VARCHAR(abc)"), None);
        // SQLite's declared type is free text, so a numeric parenthetical that means
        // something other than a length is read as one. Types that take no size in
        // practice (TEXT, INTEGER, UNSIGNED BIG INT) are unaffected.
        assert_eq!(
            sqlite_declared_size("TIMESTAMP(6) WITH TIME ZONE").as_deref(),
            Some("6")
        );
    }

    #[test]
    fn row_i64_accepts_both_json_shapes() {
        // Neither driver produces the number shape on this query path; it is covered
        // here because the helper accepts it by contract.
        assert_eq!(row_i64(Some(&serde_json::json!(255))), Some(255));
        assert_eq!(row_i64(Some(&serde_json::json!("255"))), Some(255));
        assert_eq!(row_i64(Some(&serde_json::json!(" 255 "))), Some(255));
        assert_eq!(row_i64(Some(&serde_json::json!(null))), None);
        assert_eq!(row_i64(Some(&serde_json::json!(""))), None);
        assert_eq!(row_i64(Some(&serde_json::json!(true))), None);
        assert_eq!(row_i64(None), None);
        // Beyond i64 it is not a size at all, and must not wrap into a negative one.
        assert_eq!(row_i64(Some(&serde_json::json!(u64::MAX))), None);
        assert_eq!(
            row_i64(Some(&serde_json::json!("18446744073709551615"))),
            None
        );
    }
    ...
    async fn mysql_late_errors_after_row_budget_are_not_reported_as_success() {
        ...
        let error = execute_network(config, "SELECT 1; SELECT 1/0", 500)
            .await
            .unwrap_err();
        assert!(error.contains("late database error"));
        tokio::time::timeout(Duration::from_secs(2), server)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn postgres_late_errors_and_write_batches_are_consumed_to_completion() {
        for ending in ["error", "complete"] {
            ...
            let server = tokio::spawn(postgres_rows_server(listener, vec![Some("1")], 700, ending));
            let sql = "BEGIN; UPDATE users SET id=1; SELECT id FROM users; COMMIT";
            let result = execute_network(config, sql, 500).await;
            if ending == "error" {
                assert!(result.unwrap_err().contains("late database error"));
            } else {
            ...
            assert_eq!(sent.trim_end_matches('\0'), sql);
        }
    }
    ...
    fn mysql_and_postgres_pages_use_dialect_limits_and_preserve_explicit_budget() {
        assert_eq!(
            prepare_query("select * from users", &DatabaseDriver::Mysql, 50, Some(0)),
            ("SELECT * FROM users LIMIT 0, 50".into(), true)
        );
        assert_eq!(
            prepare_query(
                "select * from users",
            ...
            ("SELECT * FROM users LIMIT 100 OFFSET 200".into(), true)
        );
        assert_eq!(
            prepare_query("select * from users", &DatabaseDriver::Mysql, 500, Some(0)),
            ("SELECT * FROM users LIMIT 0, 500".into(), true)
        );
        assert_eq!(
            prepare_query(
                "select * from users",
            ...
            ("SELECT * FROM users LIMIT 500, 500".into(), true)
        );
        assert_eq!(
            prepare_query(
                "select * from users",
            ...
            ("SELECT * FROM users LIMIT 500 OFFSET 500".into(), true)
        );
        assert_eq!(
            prepare_query(
                "select * from users LIMIT 900 OFFSET 20",
                &DatabaseDriver::Postgres,
                500,
                Some(500)
            ),
            ("SELECT * FROM users LIMIT 400 OFFSET 520".into(), false)
        );
        for sql in [
            "DELETE FROM users RETURNING *",
            "SELECT 1; UPDATE users SET id=2",
            "SELECT * INTO copied FROM users",
        ] {
            assert_eq!(
                prepare_query(sql, &DatabaseDriver::Postgres, 500, Some(0)),
                (sql.into(), false)
            );
        }
    }
    ...
    #[test]
    fn sqlite_declared_sizes_reach_the_structure_view() {
        let path = temp_db();
        let manager = DatabaseRuntimeManager::new();
        let runtime = manager
            .connect(DatabaseConnectionConfig {
                ...
                read_only: false,
                ..network_config(DatabaseDriver::Sqlite, 0)
            })
            .unwrap();
        manager
            .execute(
                &runtime.id,
                "CREATE TABLE sized(id INTEGER PRIMARY KEY, name VARCHAR(255), price DECIMAL(10,2), note TEXT)",
                500,
            )
            .unwrap();
        let columns = manager.describe_table(&runtime.id, "sized").unwrap();
        let lengths: Vec<Option<&str>> = columns
            .iter()
            .map(|column| column.length.as_deref())
            .collect();
        // SQLite keeps the size in the declared type text, and only the types that
        // declare one report it.
        assert_eq!(lengths, vec![None, Some("255"), Some("10,2"), None]);
        assert_eq!(columns[0].name, "id");
        assert!(columns[0].primary_key);
        assert_eq!(columns[1].data_type, "VARCHAR(255)");
        assert_eq!(columns[3].data_type, "TEXT");
        manager.disconnect(&runtime.id).unwrap();
    ...
    fn sqlite_page_budgets_and_optional_column_comments() {
        ...
        let result = manager
            .execute(&runtime.id, "SELECT * FROM items", 500)
            .unwrap();
        assert_eq!(result.rows.len(), 500);
        assert!(result.truncated);
        let result = manager
            .execute(&runtime.id, "SELECT * FROM items LIMIT 500", 500)
            .unwrap();
        assert_eq!(result.rows.len(), 500);
        for sql in ["VALUES (1), (2)", "SELECT 1 UNION ALL VALUES (2)"] {
            assert_eq!(
                prepare_query(sql, &DatabaseDriver::Sqlite, 500, Some(0)),
                (sql.into(), false)
            );
            let values = manager.execute_page(&runtime.id, sql, 0, 50).unwrap();
            assert_eq!(values.rows.len(), 2);
            assert_eq!(values.has_more, Some(false));
        }
        let compact = manager
            .execute_page(&runtime.id, "SELECT * FROM items ORDER BY id", 0, 50)
            .unwrap();
        assert_eq!(compact.rows.len(), 50);
        assert_eq!(compact.has_more, Some(true));
        let compact_second = manager
            .execute_page(&runtime.id, "SELECT * FROM items ORDER BY id", 50, 50)
            .unwrap();
        assert_eq!(compact_second.rows.len(), 50);
        assert_eq!(compact_second.rows[0][0], "51");
        let first = manager
            .execute_page(&runtime.id, "SELECT * FROM items ORDER BY id", 0, 500)
            .unwrap();
        let second = manager
            .execute_page(&runtime.id, "SELECT * FROM items ORDER BY id", 500, 500)
            .unwrap();
        assert_eq!(first.rows.len(), 500);
        assert_eq!(second.rows.len(), 500);
        assert_eq!(second.rows[0][0], "501");
        assert_eq!(second.has_more, Some(true));
        let end = manager
            .execute_page(&runtime.id, "SELECT * FROM items ORDER BY id", 1500, 500)
            .unwrap();
        assert!(end.rows.is_empty());
        assert_eq!(end.has_more, Some(false));
        assert!(
            manager
                .execute_page(
                    &runtime.id,
                    "INSERT INTO items VALUES(9999,'bad')",
                    500,
                    500
                )
                .is_err()
        ...
        let count = manager
            .execute(&runtime.id, "SELECT COUNT(*) FROM items", 500)
            .unwrap();
        assert_eq!(count.rows[0][0], "1500");
        let columns = manager.describe_table(&runtime.id, "items").unwrap();
        assert!(columns.iter().all(|c| c.comment.is_none()));
        let mut value = serde_json::to_value(&columns[0]).unwrap();
        let legacy_object = value.as_object_mut().unwrap();
        legacy_object.remove("comment");
        legacy_object.remove("length");
        let legacy: DatabaseColumnInfo = serde_json::from_value(value).unwrap();
        assert!(legacy.comment.is_none());
        assert!(legacy.length.is_none());
        let column = DatabaseColumnInfo {
            comment: Some("用户编号".into()),
            length: Some("255".into()),
            ..legacy
        };
        let value = serde_json::to_value(column).unwrap();
        assert_eq!(value["comment"], "用户编号");
        assert_eq!(value["length"], "255");
        manager.disconnect(&runtime.id).unwrap();
        std::fs::remove_file(path).unwrap();
    }
    ...
    async fn mysql_successful_test_and_query_close_the_connection() {
        for driver in [DatabaseDriver::Mysql, DatabaseDriver::Mariadb] {
            for test_only in [true, false] {
                ...
                let operation = async {
                    if test_only {
                        validate_network_config(config).await.unwrap();
                    } else {
                        let result = execute_network(config, "SELECT 1", 1).await.unwrap();
                        assert_eq!(result.rows, vec![vec![serde_json::json!("1")]]);
                    }
                };
                tokio::time::timeout(Duration::from_secs(3), operation)
                    .await
                    .expect("successful operation must not hang during cleanup");
                assert!(
    ...
    async fn stalled_handshakes_timeout_without_registering_a_runtime() {
        for driver in [DatabaseDriver::Mysql, DatabaseDriver::Postgres] {
            ...
            let result = tokio::time::timeout(
                ...
                tokio::task::spawn_blocking(move || worker.connect(config)),
            )
            .await
            .unwrap()
            .unwrap();
    ...
    async fn stalled_query_times_out_and_does_not_lock_runtime_directory() {
        ...
        let proxy = tokio::spawn(async move {
            ...
            let mut outgoing = tokio::net::TcpStream::connect(("127.0.0.1", proxy_port))
                .await
                .unwrap();
        ...
        tokio::time::timeout(
            ...
            tokio::task::spawn_blocking(move || {
                assert_eq!(worker.list().unwrap().len(), 1);
                worker.disconnect("fixture").unwrap();
            }),
        )
        .await
        .expect("directory and disconnect must remain responsive")
        .unwrap();
        let error = tokio::time::timeout(Duration::from_secs(33), query)
            .await
            .unwrap()
            .unwrap()
            .unwrap_err();
        ...
        assert!(
            tokio::time::timeout(Duration::from_secs(2), server)
                .await
                .expect("timed-out query must close its socket")
                .unwrap()
        );
        proxy.abort();
    }
    ...
    fn sqlite_test_does_not_create_files_and_rejects_invalid_databases() {
        ...
        assert!(test_connection(config.clone()).is_err());
        assert!(!path.exists());
    ...
    async fn postgres_rows_server(
        ...
        for i in 0..values.len() {
            description.extend_from_slice(format!("col{i}\0").as_bytes());
        ...
        for _ in 0..count {
            ...
            for value in &values {
                // A -1 length is the protocol's NULL, which is what the size columns
                // carry for most types.
                match value {
                    Some(value) => {
        ...
        if ending == "complete" {
            message(&mut stream, b'C', format!("SELECT {count}\0").as_bytes()).await;
        ...
        if ending == "error" {
            message(
                &mut stream,
                b'E',
                b"SERROR\0C22012\0Mlate database error\0\0",
            )
            .await;
            message(&mut stream, b'Z', b"I").await;
        }
    ...
    async fn postgres_row_budget_is_applied_on_server_and_completed() {
        ...
        let server = tokio::spawn(postgres_rows_server(
            listener,
            vec![Some("1")],
            500,
            "complete",
        ...
        .await
        .expect("must stop without collecting full query")
        .unwrap();
        assert_eq!(result.rows.len(), 500);
        assert!(result.truncated);
        assert_eq!(result.columns, vec!["col0"]);
        let sql = tokio::time::timeout(Duration::from_secs(2), server)
            .await
            .expect("capped query must close socket")
            .unwrap();
        assert!(sql.contains("LIMIT 500"));
    }
    ...
    async fn postgres_column_comment_survives_native_metadata_path() {
        ...
        let server = tokio::spawn(postgres_rows_server(
            ...
            vec![
                ...
                Some("0"),
                Some("用户编号"),
                // An integer column takes no declared length: PostgreSQL reports a NULL
                // character_maximum_length but real precision/scale numbers, which must
                // not be mistaken for a size.
                None,
                Some("32"),
                Some("0"),
                Some("PRI"),
        ...
        let columns =
            tokio::task::spawn_blocking(move || manager.describe_table("fixture", "users"))
                ...
                .unwrap()
                .unwrap();
        assert_eq!(columns[0].comment.as_deref(), Some("用户编号"));
        assert!(columns[0].not_null);
        assert!(columns[0].primary_key);
        // Precision 32 with scale 0 is how PostgreSQL describes a plain integer; it is
        // not a declared size and must not reach the structure view.
        assert_eq!(columns[0].length, None);
        let sql = server.await.unwrap();
        assert!(sql.contains("col_description"));
        assert!(sql.contains("attr.attnum"));
        assert!(sql.contains("c.character_maximum_length"));
        assert!(sql.contains("c.numeric_precision"));
        assert!(sql.contains("c.numeric_scale"));
        assert!(sql.contains("information_schema.table_constraints"));
        assert!(sql.contains("information_schema.key_column_usage"));
    }

    #[tokio::test]
    async fn postgres_declared_size_survives_native_metadata_path() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let config = network_config(
            DatabaseDriver::Postgres,
            listener.local_addr().unwrap().port(),
        ...
        let server = tokio::spawn(postgres_rows_server(
            ...
            vec![
                ...
                None,
                None,
                // The left join finds no primary key constraint for this column, and the
                // projected marker comes back as an empty string rather than NULL.
                Some(""),
            ],
            1,
            "complete",
        ...
        let columns =
            tokio::task::spawn_blocking(move || manager.describe_table("fixture", "users"))
                ...
                .unwrap()
                .unwrap();
        assert_eq!(columns[0].data_type, "character varying");
        assert_eq!(columns[0].length.as_deref(), Some("255"));
        assert!(!columns[0].not_null);
        assert!(!columns[0].primary_key);
        assert_eq!(columns[0].default_value, None);
        assert_eq!(columns[0].comment, None);
        assert!(server.await.unwrap().contains("character_maximum_length"));
    }
    ...
    async fn postgres_stalled_query_aborts_connection_driver_and_closes_socket() {
        ...
        let error = tokio::time::timeout(
            ...
            execute_network(config, "SELECT 1", 1),
        )
        .await
        .unwrap()
        .unwrap_err();
        ...
        tokio::time::timeout(Duration::from_secs(2), server)
            .await
            .expect("PostgreSQL connection must close after timeout")
            .unwrap();
    }
    ...
    fn postgres_config_accepts_frontend_and_legacy_driver_names_without_serializing_passwords() {
        for name in ["postgres", "postgresql"] {
            let config: DatabaseConnectionConfig = serde_json::from_value(
                serde_json::json!({ "driver": name, "password": "synthetic-fixture" }),
            )
            .unwrap();
    ...
    fn sqlite_query_is_bounded_and_import_rolls_back() {
        ...
        manager
            .import_sql(
                &runtime.id,
                "insert into items(name) values ('a'); insert into missing(name) values ('b');",
            )
            .unwrap_err();
        let result = manager
            .execute(&runtime.id, "select * from items", 1)
            .unwrap();
    ...
    fn sqlite_row_import_is_transactional() {
        ...
        let runtime = manager
            ...
            .unwrap();
        manager
            .execute(&runtime.id, "create table items(id integer, name text)", 10)
            .unwrap();
        let count = manager
            .import_rows(
                &runtime.id,
        ...
        assert!(
            manager
                .import_rows(
                    &runtime.id,
                    "items",
                    vec!["id".into(), "bad-name".into()],
                    vec![vec![serde_json::json!(2), serde_json::json!("b")]]
                )
                .is_err()
        );
        let result = manager
            .execute(&runtime.id, "select count(*) from items", 10)
            .unwrap();
    ...
    fn exports_report_table_progress_and_query_progress() {
        ...
        manager
            .execute(&runtime.id, "create table first(id integer)", 10)
            .unwrap();
        manager
            .execute(&runtime.id, "create table second(id integer)", 10)
            .unwrap();
        ...
        let dump_sink: DatabaseExportProgressSink = Arc::new(move |current, total, table| {
            dump_capture
                .lock()
                .unwrap()
                .push((current, total, table.map(str::to_owned)))
        });
        manager
            .export_sql_with_progress(&runtime.id, &dump_path, None, true, Some(dump_sink))
            .unwrap();
        assert_eq!(
            dump_events.lock().unwrap().as_slice(),
            &[
                (0, 2, None),
                (1, 2, Some("first".into())),
                (2, 2, Some("second".into()))
            ]
        );
        ...
        let query_sink: DatabaseExportProgressSink = Arc::new(move |current, total, table| {
            query_capture
                .lock()
                .unwrap()
                .push((current, total, table.map(str::to_owned)))
        });
        write_query_sql_file_with_progress(
            &query_path,
        ...
        .unwrap();
        assert_eq!(
            query_events.lock().unwrap().as_slice(),
            &[(0, 1, None), (1, 1, None)]
        );
    ...
    fn sqlite_dump_round_trips_schema_and_values_into_a_new_database() {
        ...
        manager
            .execute(&source.id, "create table other(id integer)", 10)
            .unwrap();
        manager
            .execute(&source.id, "create index items_name on items(name)", 10)
            .unwrap();
        manager
            .execute(
                &source.id,
                "insert into items values (1, 'quote''s\nline', 0.1, X'00FF41', NULL)",
                10,
            )
            .unwrap();
        ...
        manager
            .execute(
                &source.id,
                "insert into items values (2, CAST(X'FF41' AS TEXT), -2.5, NULL, 'ok')",
                10,
            )
            .unwrap();
        manager
            .execute(&source.id, "insert into other values (7)", 10)
            .unwrap();
        ...
        assert!(text.contains(
            "INSERT INTO \"items\" (\"id\", \"name\", \"ratio\", \"payload\", \"note\") VALUES"
        ));
        ...
        let projection =
            "select id, name, ratio, hex(payload), note, typeof(name) from items order by id";
        ...
        for file in [source_path, target_path, dump] {
    ...
    fn sqlite_table_dump_keeps_only_the_requested_table_and_its_indexes() {
        ...
        manager
            ...
            .unwrap();
        manager
            .execute(&runtime.id, "create table other(id integer)", 10)
            .unwrap();
        manager
            .execute(&runtime.id, "create index items_name on items(name)", 10)
            .unwrap();
        manager
            .execute(&runtime.id, "insert into items values (1, 'a')", 10)
            .unwrap();
        manager
            .execute(&runtime.id, "insert into other values (2)", 10)
            .unwrap();
        let dump = temp_db().with_extension("sql");
        let summary = manager
            .export_sql(&runtime.id, &dump, Some("items"))
            .unwrap();
        ...
        assert!(
            manager
                .export_sql(&runtime.id, &dump, Some("missing"))
                .unwrap_err()
                .contains("找不到")
        );
        for file in [path, dump] {
            let _ = std::fs::remove_file(file);
        }
    }
    ...
    fn sqlite_schema_only_dump_omits_rows_for_database_and_table_exports() {
        ...
        let summary = manager
            .export_sql_with_options(&runtime.id, &database_dump, None, true)
            .unwrap();
        ...
        let summary = manager
            .export_sql_with_options(&runtime.id, &table_dump, Some("items"), true)
            .unwrap();
        ...
        assert!(!text.contains("INSERT INTO"));
        assert!(!text.contains("other"));
        for file in [path, database_dump, table_dump] {
            let _ = std::fs::remove_file(file);
        }
    }
    ...
    fn sqlite_dump_of_an_empty_database_restores_without_error() {
        ...
        assert!(
            std::fs::read_to_string(&dump)
                .unwrap()
                .starts_with("-- Luna Mux database dump")
        );
        let (target, target_path) = sqlite_runtime(&manager);
        manager.import_sql_file(&target.id, &dump).unwrap();
        for file in [path, target_path, dump] {
            let _ = std::fs::remove_file(file);
        }
    }
    ...
    fn sqlite_import_from_file_is_all_or_nothing() {
        ...
        let (runtime, path) = sqlite_runtime(&manager);
        manager
            .execute(&runtime.id, "create table items(id integer)", 10)
            .unwrap();
        ...
        assert!(
            manager
                .import_sql_file(&runtime.id, &script)
                .unwrap_err()
        ...
        assert!(
            manager
                .import_sql_file(&runtime.id, &temp_db().with_extension("missing.sql"))
                .unwrap_err()
                .contains("无法读取 SQL 文件")
        );
        let _ = std::fs::remove_file(path);
    ...
    fn mysql_dump_values_follow_the_column_kind() {
        use mysql_async::consts::ColumnType;
        let column = |kind: ColumnType, charset: u16| {
            mysql_async::Column::new(kind).with_character_set(charset)
        };
        assert_eq!(
            mysql_value_kind(&column(ColumnType::MYSQL_TYPE_LONG, MYSQL_BINARY_CHARSET)),
            MysqlValueKind::Number
        );
        assert_eq!(
            mysql_value_kind(&column(
        ...
        assert_eq!(
            mysql_value_kind(&column(ColumnType::MYSQL_TYPE_BLOB, MYSQL_BINARY_CHARSET)),
            MysqlValueKind::Binary
        );
        assert_eq!(
            mysql_value_kind(&column(ColumnType::MYSQL_TYPE_VAR_STRING, 45)),
            MysqlValueKind::Text
        );
        ...
        for (value, kind) in [
            ...
            (
                mysql_async::Value::Bytes(b"-17.5".to_vec()),
                MysqlValueKind::Number,
            ),
            (
                mysql_async::Value::Bytes(b"it's".to_vec()),
                MysqlValueKind::Text,
            ),
            (
                mysql_async::Value::Bytes(vec![0x00, 0xff]),
                MysqlValueKind::Binary,
            ),
        ...
        assert_eq!(
            ...
            "NULL-17.5'it''s'X'00FF'X'615C62'X'6C696E650A66656564'"
        );
    }
    ...
    fn mysql_temporal_values_keep_their_text_form() {
        assert_eq!(
            mysql_datetime_text(2024, 1, 2, 3, 4, 5, 0),
            "2024-01-02 03:04:05"
        );
        assert_eq!(
            mysql_datetime_text(2024, 1, 2, 3, 4, 5, 120_000),
            "2024-01-02 03:04:05.120000"
        );
        assert_eq!(mysql_time_text(false, 1, 2, 3, 4, 0), "26:03:04");
        assert_eq!(
            mysql_time_text(true, 0, 0, 0, 1, 500_000),
            "-00:00:01.500000"
        );
    }
    ...
    fn postgres_dump_values_quote_text_and_hex_bytea() {
        ...
        assert_eq!(
            String::from_utf8(out).unwrap(),
            r"NULL'it''s'E'back\\slash'E'\\x616263'"
        );
    }
    ...
    fn postgres_column_definitions_keep_serial_identity_and_defaults() {
        let column = |name: &str, type_sql: &str, default: Option<&str>| PostgresColumn {
            ...
            serial: postgres_serial_type(type_sql, default),
            identity: false,
        };
        ...
        assert_eq!(
            column("title", "text", Some("'draft'::text")).definition(),
        ...
        assert_eq!(
            postgres_serial_type("numeric", Some("nextval('x'::regclass)")),
            None
        );
        let identity = PostgresColumn {
            identity: true,
            not_null: true,
    ...
    fn mysql_column_packet(name: &str, column_type: u8, charset: u16) -> Vec<u8> {
        ...
        column.push(name.len() as u8);
        column.extend_from_slice(name.as_bytes());
        column.push(0);
        column.push(12);
        column.extend_from_slice(&charset.to_le_bytes());
        column.extend_from_slice(&1024u32.to_le_bytes());
        column.push(column_type);
        column.extend_from_slice(&[0, 0, 0, 0, 0]);
        column
    }

    async fn mysql_result(
        ...
        let mut sequence = 2;
        for (name, kind, charset) in columns {
            packet(
                stream,
                sequence,
                &mysql_column_packet(name, *kind, *charset),
            )
            .await;
            sequence += 1;
        }
    ...
    async fn mysql_dump_server(listener: tokio::net::TcpListener) -> Vec<String> {
        ...
        while let Ok(data) = read_packet(&mut stream).await {
            ...
            if sql.contains("information_schema.tables") {
                ...
                let rows = if requested.is_some_and(|name| name != "posts") {
                    vec![]
                } else {
                    vec![vec![Some(b"posts".to_vec()), Some(b"BASE TABLE".to_vec())]]
                };
                mysql_result(
                    &mut stream,
                    &[("table_name", 253, TEXT), ("table_type", 253, TEXT)],
                    &rows,
                )
                .await;
                ...
                mysql_result(
                    ...
                    &[
                        ...
                        vec![
                            Some(b"2".to_vec()),
                            Some(b"quote's".to_vec()),
                            None,
                            Some(b"2024-05-06 07:08:09".to_vec()),
                        ],
                    ],
                )
                .await;
    ...
    async fn mysql_dump_writes_schema_and_batched_rows() {
        ...
        assert!(queries.iter().any(|sql| {
            sql.starts_with("SELECT table_name, table_type FROM information_schema.tables")
        }));
    ...
    async fn mysql_dump_of_one_table_filters_the_query_and_reports_missing_tables() {
        ...
        let export = mysql_export_sql(config, &path, Some("posts"), false)
            .await
            .unwrap();
        assert_eq!((export.tables, export.rows), (1, 2));
        assert!(
            std::fs::read_to_string(&path)
                .unwrap()
                .starts_with("-- Luna Mux dump: posts")
        );
        ...
        let queries = server
            ...
            .unwrap()
            .into_iter()
            .filter(|sql| !sql.contains("@@max_allowed_packet"))
            .collect::<Vec<_>>();
        assert!(
            queries
                .first()
                .unwrap()
                .contains("AND table_name = 'posts'")
        );
        let _ = std::fs::remove_file(path);
        ...
        let error = mysql_export_sql(config, &path, Some("missing"), false)
            .await
            .unwrap_err();
        ...
        loop {
            ...
            if sql.contains("information_schema.tables") {
                result(
                    ...
                    &[vec![Some("posts"), Some("BASE TABLE")]],
                )
                .await;
                ...
                result(
                    ...
                    &[
                        ...
                        vec![
                            Some("posts_user_fk"),
                            Some("f"),
                            Some("FOREIGN KEY (user_id) REFERENCES users(id)"),
                        ],
                    ],
                )
                .await;
                ...
                result(
                    &mut stream,
                    &["id", "title", "payload"],
                    &[vec![Some("1"), Some("hello"), Some("\\x616263")]],
                )
                .await;
            } else {
                message(&mut stream, b'C', b"SET\0").await;
                message(&mut stream, b'Z', b"I").await;
            }
            queries.push(sql);
        }
    ...
    async fn postgres_dump_writes_tables_data_constraints_and_sequences() {
        ...
        let export = postgres_export_sql(config, &path, None, false)
            .await
            .unwrap();
        ...
        assert!(
            script
                .contains("ALTER TABLE \"posts\" ADD CONSTRAINT \"posts_pkey\" PRIMARY KEY (id);")
        );
    ...
    async fn postgres_schema_only_dump_skips_data_queries_inserts_and_sequence_resets() {
        ...
        let export = postgres_export_sql(config, &path, None, true)
            .await
            .unwrap();
        assert_eq!((export.tables, export.rows), (1, 0));
        let script = std::fs::read_to_string(&path).unwrap();
