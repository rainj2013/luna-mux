...
- Prefer incremental observation over repeated full reads. The first agent_browser_snapshot establishes a baseline; pass extraArgs ["--delta"] on later snapshots in the same tab to receive only unchanged or structural deltas, and extraArgs ["--delta", "--full"] when you need to reset the baseline. Pass extraArgs ["--if-changed"] on agent_browser_screenshot when re-checking a page you already captured, so an unchanged page returns no image instead of spending tokens on an identical one. These are per-call flags, not Luna Mux defaults, so choose them deliberately.
