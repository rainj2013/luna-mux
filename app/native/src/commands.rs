...
use crate::{
    ...
    database_runtime::{
        DatabaseConnectionConfig, DatabaseExportProgressSink, DatabaseQueryResult,
        DatabaseRuntimeManager, DatabaseRuntimeSummary, DatabaseSqlExportProgress,
    },
...
async fn database_worker<T: Send + 'static>(
...
fn resolve_database_password(
    db: &Database,
    config: &mut DatabaseConnectionConfig,
) -> Result<(), String> {
    if config.password.is_some()
        || matches!(
            config.driver,
            crate::database_runtime::DatabaseDriver::Sqlite
        )
    {
        return Ok(());
    }
    let Some(id) = config.profile_id.as_deref() else {
        return Ok(());
    };
    let profile = db
    ...
    let default_port = if driver == DatabaseDriver::Postgresql {
        5432
    } else {
        3306
    };
    ...
    if profile.driver != driver
        || Some(profile.host.as_str()) != config.host.as_deref()
    ...
    if profile.has_saved_credential {
        config.password = db.get_database_credential(id);
    }
    Ok(())
}
...
mod database_command_tests {
    ...
    async fn database_worker_does_not_run_on_the_calling_thread() {
        ...
        let worker = database_worker(move || Ok(std::thread::current().id()))
            .await
            .unwrap();
    ...
    fn saved_password_resolution_checks_the_destination_and_explicit_empty_password() {
        ...
        let mut matching: DatabaseConnectionConfig =
        ...
        for (field, value) in [
            ("host", json!("192.0.2.1")),
            ("port", json!(3307)),
            ("username", json!("different")),
            ("database", json!("other")),
            ("sslEnabled", json!(false)),
            ("driver", json!("postgresql")),
        ] {
            let mut changed = original.clone();
            changed[field] = value;
            let mut config: DatabaseConnectionConfig =
                serde_json::from_value(changed.clone()).unwrap();
            assert!(
                resolve_database_password(&db, &mut config).is_err(),
        ...
        missing.profile_id = Some("missing".into());
...
#[tauri::command]
pub fn database_profiles_list(state: State<AppState>) -> Result<Vec<DatabaseProfile>, String> {
    state.db.list_database_profiles()
}

#[tauri::command]
pub fn database_profiles_save(
    state: State<AppState>,
    input: DatabaseProfileInput,
) -> Result<DatabaseProfile, String> {
    state.db.save_database_profile(input)
}

#[tauri::command]
pub fn database_profiles_remove(state: State<AppState>, id: String) -> Result<(), String> {
    state.db.remove_database_profile(&id)
}

#[tauri::command]
pub fn database_profiles_reorder(
    state: State<AppState>,
    ids: Vec<String>,
) -> Result<Vec<DatabaseProfile>, String> {
    state.db.reorder_database_profiles(&ids)
}

#[tauri::command]
pub fn database_profiles_move_to_group(
    state: State<AppState>,
    id: String,
    group: String,
) -> Result<Vec<DatabaseProfile>, String> {
    state.db.move_database_profile_to_group(&id, &group)
}
#[tauri::command]
pub async fn database_profiles_save_credential(
...
#[tauri::command]
pub async fn database_profiles_forget_credential(
    state: State<'_, AppState>,
    id: String,
) -> Result<(), String> {
    let db = state.db.clone();
...
pub fn database_pane_ui_mount(
    ...
    let pane = state
        ...
        .ok_or_else(|| "Mux Pane 不存在".to_string())?;
    if pane.kind != MuxPaneKind::Database {
        return Err("目标 Pane 不是数据库窗格".into());
    }
    state.database_pane_ui.mount(&pane_id, &mount_id, mounted)
}

#[tauri::command]
pub fn database_pane_ui_respond(
    state: State<AppState>,
    request_id: String,
    pane_id: String,
    mount_id: String,
    ...
    let result = if let Some(error) = result.get("error").and_then(|value| value.as_str()) {
        Err(error.to_string())
    } else {
    ...
    state
        .database_pane_ui
        .respond(&request_id, &pane_id, &mount_id, result)
}

#[tauri::command]
pub async fn database_pane_ui_request(
    app: AppHandle,
    state: State<'_, AppState>,
    pane_id: String,
    action: PaneUiAction,
    ...
    state
        ...
        .request(&pane_id, action, |request| {
            app.emit("database-pane-ui:request", request)
...
#[tauri::command]
pub async fn database_runtimes_list(
    state: State<'_, AppState>,
) -> Result<Vec<DatabaseRuntimeSummary>, String> {
    let manager = state.database_runtimes.clone();
    database_worker(move || manager.list()).await
}

#[tauri::command]
pub async fn database_runtime_connect(
    state: State<'_, AppState>,
    mut config: DatabaseConnectionConfig,
...
#[tauri::command]
pub async fn database_runtime_disconnect(
    state: State<'_, AppState>,
    runtime_id: String,
) -> Result<(), String> {
    let manager = state.database_runtimes.clone();
...
#[tauri::command]
pub async fn database_runtime_execute(
    state: State<'_, AppState>,
    runtime_id: String,
    sql: String,
    ...
    database_worker(move || {
        ...
        if let Some(offset) = page_offset {
            manager.execute_page(&runtime_id, &sql, offset, page_size)
        } else {
            manager.execute(&runtime_id, &sql, page_size)
        }
    })
    .await
}
...
#[tauri::command]
pub async fn database_runtime_list_tables(
    state: State<'_, AppState>,
    runtime_id: String,
...
#[tauri::command]
pub async fn database_runtime_export_csv(
    state: State<'_, AppState>,
    runtime_id: String,
    sql: String,
...
#[tauri::command]
pub async fn database_runtime_export_json(
    state: State<'_, AppState>,
    runtime_id: String,
    sql: String,
) -> Result<String, String> {
...
#[tauri::command]
pub async fn database_runtime_write_query_sql(
    app: AppHandle,
    operation_id: Option<String>,
    path: String,
    sql: String,
    rows: u64,
) -> Result<crate::database_runtime::DatabaseSqlExport, String> {
    let progress = operation_id.map(|id| export_progress_sink(app, id));
    database_worker(move || {
        crate::database_runtime::write_query_sql_file_with_progress(
            Path::new(&path),
            &sql,
            rows,
            progress,
...
#[tauri::command]
pub async fn database_runtime_describe_table(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
) -> Result<Vec<crate::database_runtime::DatabaseColumnInfo>, String> {
    let manager = state.database_runtimes.clone();
    database_worker(move || manager.describe_table(&runtime_id, &table)).await
}

#[tauri::command]
pub async fn database_runtime_export_sql(
    app: AppHandle,
    state: State<'_, AppState>,
    runtime_id: String,
    operation_id: Option<String>,
    ...
    let progress = operation_id.map(|id| export_progress_sink(app, id));
    database_worker(move || {
        manager.export_sql_with_progress(
            &runtime_id,
            Path::new(&path),
            table.as_deref(),
            schema_only,
...
#[tauri::command]
pub async fn database_runtime_import_sql_file(
    state: State<'_, AppState>,
    runtime_id: String,
    path: String,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn database_runtime_import_rows(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
    columns: Vec<String>,
...
#[tauri::command]
pub async fn database_protocol_connect(
    mut config: DatabaseConnectionConfig,
...
#[tauri::command]
pub async fn database_runtime_add_column(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
    column: String,
...
#[tauri::command]
pub async fn database_runtime_drop_column(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
    column: String,
...
#[tauri::command]
pub async fn database_runtime_list_indexes(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
) -> Result<Vec<crate::database_runtime::DatabaseIndexInfo>, String> {
    let manager = state.database_runtimes.clone();
    database_worker(move || manager.list_indexes(&runtime_id, &table)).await
}
#[tauri::command]
pub async fn database_runtime_list_foreign_keys(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
) -> Result<Vec<crate::database_runtime::DatabaseForeignKeyInfo>, String> {
    let manager = state.database_runtimes.clone();
    database_worker(move || manager.list_foreign_keys(&runtime_id, &table)).await
}
#[tauri::command]
pub async fn database_runtime_create_index(
    state: State<'_, AppState>,
    runtime_id: String,
    table: String,
    index: String,
    ...
    let manager = state.database_runtimes.clone();
    database_worker(move || manager.create_index(&runtime_id, &table, &index, &column, unique))
        .await
}
#[tauri::command]
pub async fn database_runtime_drop_index(
    state: State<'_, AppState>,
    runtime_id: String,
    index: String,
) -> Result<u64, String> {
