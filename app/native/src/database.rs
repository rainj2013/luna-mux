...
impl Database {
    ...
    pub fn save_database_profile(
        ...
        let sort_order = existing
            .as_ref()
            .map(|p| p.sort_order)
            .unwrap_or(self.with_conn(|db| {
                db.query_row(
                    "SELECT COALESCE(MAX(sortOrder),-1)+1 FROM database_profiles",
                    [],
                    |r| r.get(0),
                )
            })?);
        let profile = DatabaseProfile {
            ...
            database_name: input.database_name.trim().into(),
            group_name: input.group_name.trim().into(),
            favorite: input.favorite,
            sort_order,
            ssh_bookmark_id: input.ssh_bookmark_id.trim().into(),
    ...
    pub fn save_database_credential(&self, id: &str, password: &str) -> Result<(), String> {
        if password.is_empty() {
            return Err("数据库密码不能为空".into());
        }
        let account = format!("database:{id}");
        Entry::new(&self.credential_service, &account)
    ...
    pub fn forget_database_credential(&self, id: &str) -> Result<(), String> {
        let account = format!("database:{id}");
        match Entry::new(&self.credential_service, &account)
            .map_err(|e| e.to_string())?
            .delete_credential()
        {
        ...
        self.with_conn(|db| {
            db.execute(
                "DELETE FROM database_credential_refs WHERE profileId=?",
                [id],
            )
            .map(|_| ())
        })
    ...
    pub fn reorder_database_profiles(
        &self,
        ids: &[String],
    ) -> Result<Vec<DatabaseProfile>, String> {
        let existing = self.list_database_profiles()?;
        let known = existing
            .iter()
            .map(|p| p.id.as_str())
            .collect::<std::collections::HashSet<_>>();
        if ids.len() != known.len() || ids.iter().any(|id| !known.contains(id.as_str())) {
            return Err("数据库连接排序列表不完整".into());
        }
        self.with_conn(|db| {
            let tx = db.transaction()?;
            for (order, id) in ids.iter().enumerate() {
                tx.execute(
                    "UPDATE database_profiles SET sortOrder=?,updatedAt=? WHERE id=?",
                    rusqlite::params![order as i64, Utc::now().to_rfc3339(), id],
                )?;
            }
            tx.commit()
        })?;
        self.list_database_profiles()
    }

    pub fn move_database_profile_to_group(
        &self,
        id: &str,
        group: &str,
    ) -> Result<Vec<DatabaseProfile>, String> {
        let group = group.trim();
        if !self.list_database_profiles()?.iter().any(|p| p.id == id) {
            return Err("数据库连接不存在".into());
        }
        self.with_conn(|db| {
            db.execute(
                "UPDATE database_profiles SET groupName=?,updatedAt=? WHERE id=?",
                rusqlite::params![group, Utc::now().to_rfc3339(), id],
            )
            .map(|_| ())
        })?;
        self.list_database_profiles()
