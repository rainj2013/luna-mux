...
| 编号 | 状态 | 任务 | 验收条件 |
| --- | --- | --- | --- |
| M0.1 | DONE | 基于 Luna Remote 历史创建独立仓库 | Luna Mux 仓库独立存在，来源仓库不被修改 |
| M0.2 | DONE | 将 Luna Remote 配置为只读 `upstream` | 分支不跟踪上游，推送地址被禁用 |
...
| M0.7 | DONE | 建立设计、任务、进度和上游规则文档 | 四份长期文档存在并互相引用 |
...
| M7.3 | TODO | 定期审阅 Luna Remote 上游 | 同步记录包含已移植、待处理和拒绝提交及证据 |
