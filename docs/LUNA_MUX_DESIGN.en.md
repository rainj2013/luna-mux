...
Browser Runtime enables Chrome WebMCP interfaces; pages must register their own tools. Local and remote Agents share MCP tool groups for page interaction, network debugging, tabs, WebMCP, cookies and storage, the encrypted auth vault, React inspection, and device emulation. React inspection requires enabling `react-devtools` when opening the page. Luna Mux does not bundle upstream skills because their CLI and named-session guidance conflicts with the managed browser contract.

Injected Agent instructions cover batch calls, accessibility audits, screenshot and snapshot comparisons, incremental observation, and output filtering. Agents choose these capabilities according to the task.
