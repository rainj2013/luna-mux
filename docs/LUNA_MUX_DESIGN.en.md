...
A Browser Resource may optionally seed its disposable profile from the local Chrome profile’s cookie state so the Agent opens pages already signed in. The switch is stored per Resource, off by default, applied only on explicit user confirmation, and reads the source profile without modifying it. The copy is an allowlist and deliberately narrow, down to file level: `Cookies`, `Network/Cookies` (Chrome 96 on), and the `Local State` Windows needs to decrypt them, which necessarily carries the DPAPI-wrapped cookie key with it. Non-cookie neighbours inside `Network` (HSTS, trust tokens, reporting state), site storage, extensions, browsing history, saved passwords, and payment data are all excluded. Site storage is not needed to reach a signed-in page and dominates the size — IndexedDB alone runs to hundreds of megabytes in a profile in daily use, and every browser start would have to copy it. Because the directory itself is disposable, each start seeds afresh and the snapshot cannot go stale. Deleting a Browser Resource or a Session removes its download directory, along with any profile directory left at the retired Session-scoped path.
...
Browser Runtime enables the Chrome WebMCP page API and testing interfaces used by preview clients (`WebMCP,WebMCPTesting`). The MCP tool profile shared by local and remote Agents includes `webmcp` for discovering and invoking page tools, retrieving detached results, and cancelling calls. Pages must register their own tools; availability depends on the Chrome and page API versions. The profile also includes `state`, exposing cookies, storage, and the encrypted auth vault (`agent_browser_auth_save`/`auth_login` and related tools) so Agents can sign in without typing passwords into a login form. That profile also brings three `skills` tools that read bundled skills, and Luna Mux bundles none: the upstream core skill teaches the `agent-browser` command line and a named session per task, which directly contradicts a product that owns the browser process and pins every call to the injected default session, and the remaining skills are Vercel-internal workflows. Installing them would answer those tools with guidance that contradicts the contract the Agent was given. `AGENT_BROWSER_SKILLS_DIR` therefore points at an empty directory in the product data root, so the tools report that no skills are bundled instead of returning install advice a packaged app cannot follow. `react` and `mobile` add component-tree inspection and device emulation. The React tools stay inert until a page is opened with `--enable react-devtools`, which is an agent-browser option rather than a Chrome flag: Chrome is launched by Luna Mux and cannot receive it, so the injected instructions tell Agents to pass it when they need React work. It installs a document script, so one open enables the tools for the rest of the browser session without relaunching. Luna Mux ships no setting to ignore HTTPS certificate errors. `--ignore-https-errors` (and `AGENT_BROWSER_IGNORE_HTTPS_ERRORS`) is a daemon-level option: passing it as per-call `extraArgs` after the MCP connection is already established did not change certificate handling, and the navigation still failed with `net::ERR_CERT_AUTHORITY_INVALID`. The binary contains `Security.setIgnoreCertificateErrors`, so the policy is delivered over CDP rather than fixed when Chrome launches, which means adding the flag to the `mcp` arguments should work; confirming that needs a real machine, so the setting is not shipped. Luna Mux starts the MCP server with the global flag `--input-mode human` so pointer motion is not treated as automation by sites or recorded runs; that flag is not a config-file key and must precede the subcommand. Loading the tool profile does not by itself reveal which capabilities are worth using: visual and structural regression (`diff screenshot` / `diff snapshot`), axe-core accessibility audits, `batch` calls, and the snapshot narrowing fields `interactive`/`selector`/`depth`/`compact` are all introduced by the prompt injected into Agents. Per-call behaviour such as `ifChanged`, `delta`, and `full` likewise cannot be enforced at session level and remains the Agent's choice on each call.
