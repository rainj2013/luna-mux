<div align="center">
  ...
  <p>
    <a href="https://github.com/rainj2013/luna-mux/releases">下载</a>
    ·
...
数据库窗格可在当前会话中连接 SQLite、MySQL / MariaDB 和 PostgreSQL，浏览表与视图、查看字段/索引/外键、执行分页 SQL 查询，并导入导出 CSV、JSON 与 SQL；SQLite 还支持部分结构编辑。

- 每个数据库窗格使用独立连接；连接配置可保存，密码保存在系统凭据库。
- 窗格默认以只读模式打开，DML 和 DDL 写入需要经过当前会话授权确认。
